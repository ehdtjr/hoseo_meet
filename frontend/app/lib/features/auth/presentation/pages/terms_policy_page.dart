    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: const TabBarView(
          children: [
            _TermsView(text: _termsText),
            _TermsView(text: _locationText),
          ],
        ),
      ),
    );
...
class _TermsView extends StatelessWidget {
  final String text;
  const _TermsView({required this.text});
...
// ───── 예시 텍스트 (필요시 실제 약관으로 교체) ─────
const String _termsText = '''
[서비스 이용약관]

제1조(목적)

이 약관은 캠퍼스밋 (이하 '회사' 라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
이 약관은 캠퍼스밋(이)가 제공하는 모든 인터넷서비스에 게시하여 공시합니다. '회사'는 ‘전자상거래 등에서의 소비자보호에 관한 법률(이하 '전자상거래법'이라 함)', '약관의 규제에 관한 법률(이하 '약관규제법'이라 함)', '전자문서 및 전자거래 기본법(이하 '전자문서법'이라 함)', ‘전자금융거래법 ‘, '정보통신망 이용촉진 및 정보보호 등에 관한 법률(이하 '정보통신망법'이라 함)', '소비자기본법' 등 관계 법령(이하 '관계법령' 이라 함)에 위배되지 않는 범위 내에서 이 약관을 변경할 수 있으며, 회사는 약관이 변경되는 경우에 변경된 약관의 내용과 시행일을 정하여, 그 시행일로부터 최소 7일 (이용자에게 불리하거나 중대한 사항의 변경은 30일) 이전부터 시행일 후 상당한 기간 동안 공지하고, 기존 이용자에게는 변경된 약관, 적용일자 및 변경사유(변경될 내용 중 중요사항에 대한 설명을 포함)를 별도의 전자적 수단(전자우편, 문자메시지, 서비스 내 전자쪽지발송, 알림 메시지를 띄우는 등의 방법)으로 개별 통지합니다. 변경된 약관은 공지하거나 통지한 시행일로부터 효력이 발생합니다.
...
본 약관은 2025.05.16.부터 시행됩니다. 
...
''';

const String _locationText = '''
[위치정보 이용약관]

제1조(목적)

본 약관은 회원(의 서비스 약관에 동의한 자를 말하며 이하 '회원'이라고 합니다)이 (이하 '회사'라고 합니다)가 제공하는 웹페이지 및 '캠퍼스밋' (회사가 개발 운영하는 모바일 애플리케이션을 말합니다 이하 '모바일앱'이라고 합니다)의 서비스를 이용함에 있어 회원과 회사의 권리 및 의무, 기타 제반 사항을 정하는 것을 목적으로 합니다.
