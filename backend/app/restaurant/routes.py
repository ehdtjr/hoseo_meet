...
from app.restaurant.crud import RestaurantPostCRUD, get_restaurant_post_crud, \
    RestaurantPostVersionCRUD, get_restaurant_post_version_crud
...
async def get_restaurants_list(
    ...
    user: User = Depends(current_active_user),
    db: AsyncSession = Depends(get_async_session),
    restaurant_crud: RestaurantPostCRUD= Depends(get_restaurant_post_crud),
):
    return await restaurant_crud.list(
        db=db,
        user_id=user.id,
        skip=skip,
        limit=limit,
