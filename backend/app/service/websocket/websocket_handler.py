...
class WebSocketEventHandler:
    ...
    async def handle_connection(self, websocket: WebSocket, user_id: int, disconnect_event: asyncio.Event):
        """
        ...
        try:
            await asyncio.gather(
                self._receive_from_redis_and_send_to_ws(websocket, queue_key, disconnect_event),
                self._receive_from_ws_and_handle_disconnect(websocket, disconnect_event)
            )
        finally:
    ...
    async def _receive_from_redis_and_send_to_ws(self, websocket: WebSocket, queue_key: str, disconnect_event: asyncio.Event):
        """
        ...
        while not disconnect_event.is_set():
            try:
                events = await pool.xread(streams={queue_key: last_event_id}, block=0, count=100)
                if events:
                    logger.debug(f"Redis에서 이벤트 수신: {events}")
    ...
    async def _receive_from_ws_and_handle_disconnect(self, websocket: WebSocket, disconnect_event: asyncio.Event):
        """
