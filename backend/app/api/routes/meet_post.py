...
@router.get("/search", response_model=Optional[list[MeetPostListResponse]])
async def get_filtered_meet_posts(
    ...
    result = await meet_post_service.get_filtered_meet_posts(
        db, user, title, type, content, skip, limit)
    return result
...
async def get_meet_post_detail(
    ...
    detail = await meet_post_service.get_detail_meet_post(
        db=db,
        user=user,
