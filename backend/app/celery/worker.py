from celery import Celery
from celery.schedules import crontab

from app.core.config import settings

app = Celery('app',
             broker=f"redis://{settings.REDIS_HOST}:{settings.REDIS_PORT}")

app.autodiscover_tasks(['app.celery.tasks', 'app.celery.event_fcm_task',
'app.celery.event_email_task', 'app.celery.story_post_task', 'app.celery.tag_task'])

app.conf.beat_schedule = {
    'garbage-collection-every-10-minutes': {
        'task': 'app.celery.tasks.run_garbage_collection',  # tasks.py에 정의될 작업
        'schedule': crontab(minute='*/10'),  # 10분마다 실행
    },
    'delete-expired-story-posts-every-hour': {
        'task': 'app.celery.story_post_task.delete_expired_story_posts_task',
        'schedule': crontab(minute='*/30'),  # 10분마다 실행
    },
    'process_user_tag': {
        'task': 'app.celery.tag_task.process_user_tag',
        'schedule': crontab(minute='*/1')
    }
}